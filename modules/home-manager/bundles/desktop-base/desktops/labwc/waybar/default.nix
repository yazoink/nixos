{
  ...
  workspacesModule = ''
    "ext/workspaces": {
      ...
      "on-scroll-up": "${lib.getExe pkgs.wtype} -M win -M ctrl -P left -p left -m shift -s 500 -m ctrl",
      "on-scroll-down": "${lib.getExe pkgs.wtype} -M win -M ctrl -P right -p right -m shift -s 500 -m ctrl",
      "format": "{name}"
    },
    "custom/workspaces": {
      "tooltip": true,
