{
  ...
  workspacesModule = ''
    "ext/workspaces": {
      ...
      "on-scroll-up": "${lib.getExe pkgs.wtype} -M win -M ctrl -P left -p left -m shift -s 500 -m win",
      "on-scroll-down": "${lib.getExe pkgs.wtype} -M win -M ctrl -P right -p right -m shift -s 500 -m win",
      "format": "{name}"
    },
    "custom/workspaces": {
      "tooltip": true,
